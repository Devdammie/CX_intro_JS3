const drugs = [
   ...
   const antibiotics = drugs.filter(category =>  category.category === "Antibiotics")
   ...
   function drugsCat (category){
     const catDrugs = drugs.map(drug => drug.category); 
     console.log(catDrugs);
   } 
   console.log(drugsCat("Antibiotics"));  
   
   //4. Log each drug’s name and its manufacturer.
   const nameAndManufacturer = drugs.forEach(drug =>{
  ...
  drugs.forEach(drug => {
   ...
   const presFalse = drugs.filter(drug => !drug.isPrescriptionOnly)
   console.log(presFalse);
   
   
  // 9. Write a function that takes a manufacturer name and returns how many drugs are from that company.
  function manufacturerDrugs (manufacturerName){
    return drugs.filter(drug => drug.manufacturer === manufacturerName).length
  }
  console.log(manufacturerDrugs("Bayer"));
   
   
  // 10. Use forEach() to count how many drugs are Analgesics.
  let analgestics = 0;
   drugs.forEach(drug => {
     if (drug.category === "Analgestic"){
        analgestics++;
     }  
   });
   console.log(`Analgestic drugs: ${analgestics}`);
